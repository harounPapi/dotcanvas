...
Assist has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.assist/userdata/logs/server.trace.ndjson`).
...
```bash
export ASSIST_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export ASSIST_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export ASSIST_OTLP_SERVICE_NAME=assist-local
```

Optional:

```bash
export ASSIST_TRACE_MIN_LEVEL=Info
export ASSIST_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `ASSIST_OTLP_*`.

macOS app bundle example:

```bash
ASSIST_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
ASSIST_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
ASSIST_OTLP_SERVICE_NAME=assist-desktop \
...
```bash
tail -f "$ASSIST_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$ASSIST_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$ASSIST_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `assist-local`, `assist-dev`, or `assist-desktop`
...
- `ASSIST_OTLP_TRACES_URL` was not set
...
- `ASSIST_TRACE_FILE`: override trace file path
- `ASSIST_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `ASSIST_TRACE_MAX_FILES`: rotated file count, default `10`
- `ASSIST_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `ASSIST_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `ASSIST_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `ASSIST_OTLP_TRACES_URL`: OTLP trace endpoint
- `ASSIST_OTLP_METRICS_URL`: OTLP metric endpoint
- `ASSIST_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `ASSIST_OTLP_SERVICE_NAME`: service name, default `assist-server`
