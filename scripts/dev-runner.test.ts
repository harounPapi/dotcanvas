...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit ASSIST_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "ASSIST_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid ASSIST_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults ASSIST_HOME to ~/.assist when not provided", () =>
      Effect.gen(function* () {
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ASSIST_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.ASSIST_NO_BROWSER, "1");
        assert.equal(env.ASSIST_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.ASSIST_LOG_WS_EVENTS, "1");
        assert.equal(env.ASSIST_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            ASSIST_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          assistHome: undefined,
        ...
        assert.equal(env.ASSIST_MODE, "web");
        assert.equal(env.ASSIST_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ASSIST_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom assistHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.ASSIST_HOME, resolve("/tmp/my-assist"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ASSIST_PORT, undefined);
        assert.equal(env.ASSIST_AUTH_TOKEN, undefined);
        assert.equal(env.ASSIST_MODE, undefined);
        assert.equal(env.ASSIST_NO_BROWSER, undefined);
        assert.equal(env.ASSIST_HOST, undefined);
