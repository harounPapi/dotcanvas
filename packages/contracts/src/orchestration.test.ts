...
it.effect("decodes capability mentions in thread.turn.start messages", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeThreadTurnStartCommand({
      type: "thread.turn.start",
      commandId: "cmd-turn-capabilities",
      threadId: "thread-1",
      message: {
        messageId: "msg-capabilities",
        role: "user",
    ...
    assert.deepStrictEqual(parsed.message.capabilityMentions, [
      {
        kind: "skill",
        token: "$obsidian-cli",
        label: "$obsidian-cli",
