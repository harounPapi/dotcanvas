...
export const ASSIST_AGENTS_RELATIVE_PATH = "AGENTS.md";
export const ASSIST_CONTEXT_DIRECTORY = ".context";
export const ASSIST_PROJECT_OVERVIEW_RELATIVE_PATH = `${ASSIST_CONTEXT_DIRECTORY}/project-overview.md`;
export const ASSIST_MEMORY_RELATIVE_PATH = `${ASSIST_CONTEXT_DIRECTORY}/memory.md`;
export const ASSIST_WORKSPACE_MAP_RELATIVE_PATH = `${ASSIST_CONTEXT_DIRECTORY}/workspace-map.md`;
export const ASSIST_OPEN_QUESTIONS_RELATIVE_PATH = `${ASSIST_CONTEXT_DIRECTORY}/open-questions.md`;
export const ASSIST_BOOTSTRAP_MARKER_RELATIVE_PATH = `${ASSIST_CONTEXT_DIRECTORY}/bootstrap-state.txt`;
export const ASSIST_BOOTSTRAP_THREAD_TITLE = "Project Bootstrap";

export const ASSIST_REQUIRED_SCAFFOLD_PATHS: ReadonlyArray<{
  relativePath: string;
  kind: ProjectEntryKind;
}> = [
  { relativePath: ASSIST_AGENTS_RELATIVE_PATH, kind: "file" },
  { relativePath: ASSIST_PROJECT_OVERVIEW_RELATIVE_PATH, kind: "file" },
  { relativePath: ASSIST_MEMORY_RELATIVE_PATH, kind: "file" },
  { relativePath: ASSIST_WORKSPACE_MAP_RELATIVE_PATH, kind: "file" },
  { relativePath: ASSIST_OPEN_QUESTIONS_RELATIVE_PATH, kind: "file" },
] as const;
...
export function buildAssistBootstrapMarkerContents(input: { bootstrapped: boolean }): string {
  const value = input.bootstrapped ? "true" : "false";
  return `bootstraped=${value}\nbootstrapped=${value}\n`;
}

export function parseAssistBootstrapMarkerContents(contents: string): boolean {
...
export function buildAssistAgentsTemplate(input: { projectTitle: string }): string {
  return `# AGENTS.md
...
- \`${ASSIST_PROJECT_OVERVIEW_RELATIVE_PATH}\`: project mission, desired artifact, constraints, success criteria, important stakeholders, deadlines, and current objective.
- \`${ASSIST_MEMORY_RELATIVE_PATH}\`: durable working memory such as key facts, assumptions, glossary, decisions, contradictions, and recent changes.
- \`${ASSIST_WORKSPACE_MAP_RELATIVE_PATH}\`: the workspace map and corpus map: how the root workspace is organized, which files/folders matter, what source material exists, active workstreams, outputs, and any structural gaps.
- \`${ASSIST_OPEN_QUESTIONS_RELATIVE_PATH}\`: unresolved questions, missing evidence, blocked decisions, and follow-ups needed from the user or the workspace.
...
export function buildAssistBootstrapDeveloperInstructions(input: { projectTitle: string }): string {
  return `You are bootstrapping a .assist project room for "${input.projectTitle}".

.assist is a project-room harness for source-heavy, evolving, outcome-driven work that must become a reliable artifact. The user should feel like they opened a room that already understands the work they are trying to complete.
...
export function buildAssistProjectOverviewTemplate(input: { projectTitle: string }): string {
...
export function buildAssistMemoryTemplate(): string {
  return `# Memory
...
export function buildAssistWorkspaceMapTemplate(): string {
...
export function buildAssistOpenQuestionsTemplate(): string {
...
export function buildAssistBootstrapFiles(input: { projectTitle: string }): ReadonlyArray<{
  relativePath: string;
  contents: string;
}> {
  return [
    {
      relativePath: ASSIST_AGENTS_RELATIVE_PATH,
      contents: `${buildAssistAgentsTemplate(input).trimEnd()}\n`,
    },
    {
      relativePath: ASSIST_PROJECT_OVERVIEW_RELATIVE_PATH,
      contents: `${buildAssistProjectOverviewTemplate(input).trimEnd()}\n`,
    },
    {
      relativePath: ASSIST_MEMORY_RELATIVE_PATH,
      contents: `${buildAssistMemoryTemplate().trimEnd()}\n`,
    },
    {
      relativePath: ASSIST_WORKSPACE_MAP_RELATIVE_PATH,
      contents: `${buildAssistWorkspaceMapTemplate().trimEnd()}\n`,
    },
    {
      relativePath: ASSIST_OPEN_QUESTIONS_RELATIVE_PATH,
      contents: `${buildAssistOpenQuestionsTemplate().trimEnd()}\n`,
    },
    {
      relativePath: ASSIST_BOOTSTRAP_MARKER_RELATIVE_PATH,
      contents: buildAssistBootstrapMarkerContents({ bootstrapped: false }),
    },
  ] as const;
}
