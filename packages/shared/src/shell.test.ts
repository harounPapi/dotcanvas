...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__ASSIST_ENV_PATH_START__\n/a:/b\n__ASSIST_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__ASSIST_ENV_PATH_START__");
    expect(args?.[1]).toContain("__ASSIST_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__ASSIST_ENV_PATH_END__",
        "__ASSIST_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__ASSIST_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      ["__ASSIST_ENV_CUSTOM_VAR_START__", "  padded value  ", "__ASSIST_ENV_CUSTOM_VAR_END__"].join(
        "\n",
      ),
    );
