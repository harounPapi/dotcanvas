...
import {
  COMPOSER_INLINE_CAPABILITY_CHIP_CLASS_NAME,
  COMPOSER_INLINE_CAPABILITY_CHIP_TOKEN_CLASS_NAME,
...
type SerializedComposerCapabilityMentionNode = Spread<
  {
    token: string;
    label: string;
    path: string;
    type: "composer-capability-mention";
    version: 1;
  },
  SerializedTextNode
>;
...
class ComposerCapabilityMentionNode extends TextNode {
  __token: string;
  __label: string;
  __path: string;

  static override getType(): string {
    return "composer-capability-mention";
  }

  static override clone(node: ComposerCapabilityMentionNode): ComposerCapabilityMentionNode {
    return new ComposerCapabilityMentionNode(node.__token, node.__label, node.__path, node.__key);
  }

  static override importJSON(
    serializedNode: SerializedComposerCapabilityMentionNode,
  ): ComposerCapabilityMentionNode {
    return $createComposerCapabilityMentionNode({
      kind: "skill",
      token: serializedNode.token,
      label: serializedNode.label,
      name: serializedNode.label,
      path: serializedNode.path,
    });
  }

  constructor(token: string, label: string, path: string, key?: NodeKey) {
    super(token, key);
    this.__token = token;
    this.__label = label;
    this.__path = path;
  ...
  override exportJSON(): SerializedComposerCapabilityMentionNode {
    return {
      ...super.exportJSON(),
      token: this.__token,
      label: this.__label,
  ...
  override createDOM(_config: EditorConfig): HTMLElement {
    const dom = document.createElement("span");
    dom.className = COMPOSER_INLINE_CAPABILITY_CHIP_CLASS_NAME;
    dom.contentEditable = "false";
    dom.setAttribute("spellcheck", "false");
    renderCapabilityMentionChipDom(dom, this.__label, this.__path);
    return dom;
  }

  override updateDOM(
    prevNode: ComposerCapabilityMentionNode,
    dom: HTMLElement,
    _config: EditorConfig,
  ): boolean {
    dom.contentEditable = "false";
...
function $createComposerCapabilityMentionNode(
  mention: Extract<ComposerCapabilityMention, { kind: "skill" }>,
): ComposerCapabilityMentionNode {
  const displayLabel = mention.label.trim().replace(/^\$/, "") || mention.name || mention.token;
  return $applyNodeReplacement(
    new ComposerCapabilityMentionNode(mention.token, displayLabel, mention.path),
  );
}
...
function isComposerInlineTokenNode(candidate: unknown): candidate is ComposerInlineTokenNode {
  return (
    ...
    candidate instanceof ComposerCapabilityMentionNode ||
...
function renderCapabilityMentionChipDom(
  ...
  container.style.setProperty("-webkit-user-select", "none");
  container.title = pathValue;
  container.dataset.capabilityPath = pathValue;

  const token = document.createElement("span");
  token.className = COMPOSER_INLINE_CAPABILITY_CHIP_TOKEN_CLASS_NAME;
  token.textContent = "$";
...
function getAbsoluteOffsetForPoint(node: LexicalNode, pointOffset: number): number {
  ...
  if ($isTextNode(node)) {
    if (node instanceof ComposerMentionNode || node instanceof ComposerCapabilityMentionNode) {
...
function $setComposerEditorPrompt(
  ...
  const segments = splitPromptIntoComposerSegments(prompt, terminalContexts, capabilityMentions);
  for (const segment of segments) {
    if (segment.type === "mention") {
      paragraph.append($createComposerMentionNode(segment.path));
      continue;
    }
    if (segment.type === "capability-mention") {
      paragraph.append($createComposerCapabilityMentionNode(segment.mention));
      continue;
    }
...
function ComposerInlineTokenArrowPlugin(props: {
  ...
  }, [editor, props.capabilityMentions]);
...
function ComposerPromptEditorInner({
  ...
  const initialCursor = clampCollapsedComposerCursor(value, cursor, capabilityMentions);
  ...
  const capabilityMentionsSignature = useMemo(
    () =>
      capabilityMentions
        .map((mention) =>
          mention.kind === "skill"
            ? `skill\u001f${mention.token}\u001f${mention.label}\u001f${mention.path}`
            : `app\u001f${mention.token}\u001f${mention.label}\u001f${mention.appId}`,
        )
        .join("\u001e"),
    [capabilityMentions],
  );
  const capabilityMentionsSignatureRef = useRef(capabilityMentionsSignature);
  const capabilityMentionsRef = useRef(capabilityMentions);
  const snapshotRef = useRef({
    value,
    cursor: initialCursor,
    expandedCursor: expandCollapsedComposerCursor(value, initialCursor, capabilityMentions),
  ...
  useEffect(() => {
    capabilityMentionsRef.current = capabilityMentions;
  }, [capabilityMentions]);
  ...
  useLayoutEffect(() => {
    const normalizedCursor = clampCollapsedComposerCursor(value, cursor, capabilityMentions);
    const previousSnapshot = snapshotRef.current;
    const contextsChanged = terminalContextsSignatureRef.current !== terminalContextsSignature;
    const capabilityMentionsChanged =
      capabilityMentionsSignatureRef.current !== capabilityMentionsSignature;
    if (
      previousSnapshot.value === value &&
      previousSnapshot.cursor === normalizedCursor &&
      !contextsChanged &&
      !capabilityMentionsChanged
    ) {
      return;
    }
    ...
    snapshotRef.current = {
      ...
      expandedCursor: expandCollapsedComposerCursor(value, normalizedCursor, capabilityMentions),
      terminalContextIds: terminalContexts.map((context) => context.id),
    };
    ...
    editor.update(() => {
      const shouldRewriteEditorState =
        previousSnapshot.value !== value || contextsChanged || capabilityMentionsChanged;
      if (shouldRewriteEditorState) {
        $setComposerEditorPrompt(value, terminalContexts, capabilityMentions);
      }
  ...
  return (
    <ComposerTerminalContextActionsContext.Provider value={terminalContextActions}>
      <div className="relative">
        ...
        <ComposerInlineTokenArrowPlugin capabilityMentions={capabilityMentions} />
...
>(function ComposerPromptEditor(
  ...
  const initialCapabilityMentionsRef = useRef(capabilityMentions);
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "t3tools-composer-editor",
      editable: true,
      nodes: [ComposerMentionNode, ComposerCapabilityMentionNode, ComposerTerminalContextNode],
      editorState: () => {
        $setComposerEditorPrompt(
          initialValueRef.current,
          initialTerminalContextsRef.current,
          initialCapabilityMentionsRef.current,
        );
      },
      onError: (error) => {
        throw error;
