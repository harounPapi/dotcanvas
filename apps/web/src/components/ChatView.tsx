...
const COMPOSER_CAPABILITY_QUERY_DEBOUNCE_MS = 120;
const COMPOSER_CAPABILITY_LIMIT = 40;
...
function capabilityMentionKey(mention: ComposerCapabilityMention): string {
  return mention.kind === "skill"
    ? `skill\u0000${mention.token}\u0000${mention.path}`
    : `app\u0000${mention.token}\u0000${mention.appId}`;
}

function resolveCapabilityMentionsForPrompt(
  prompt: string,
  mentions: ReadonlyArray<ComposerCapabilityMention>,
): ComposerCapabilityMention[] {
  const seen = new Set<string>();
  const resolved: ComposerCapabilityMention[] = [];
  for (const mention of mentions) {
    if (!prompt.includes(mention.token)) {
      continue;
    }
    const key = capabilityMentionKey(mention);
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
...
function capabilityAppTokenLabel(name: string, appId: string): string {
...
function formatCapabilityPluginDisplayName(pluginName: string): string {
...
function stripCapabilityPluginPrefix(name: string, pluginName: string): string {
...
function capabilitySkillSourceLabel(
  ...
  if (scope === "plugin") {
    const displayName = pluginName ? formatCapabilityPluginDisplayName(pluginName) : "Plugin";
    return `From ${displayName} plugin`;
  }
  return "Global skill";
}

function capabilityCountsLabel(skillsCount: number, appsCount: number): string {
  const skillLabel = `${skillsCount} skill${skillsCount === 1 ? "" : "s"}`;
  const appLabel = `${appsCount} app${appsCount === 1 ? "" : "s"}`;
  return `${skillLabel} · ${appLabel}`;
}
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const composerCapabilityMentions = composerDraft.capabilityMentions;
  ...
  const addComposerDraftCapabilityMention = useComposerDraftStore(
    (store) => store.addCapabilityMention,
  );
  const syncComposerDraftCapabilityMentions = useComposerDraftStore(
    (store) => store.syncCapabilityMentions,
  );
  ...
  const composerCapabilityMentionsRef = useRef<ComposerCapabilityMention[]>(
  ...
  const [composerCapabilityBundleSelection, setComposerCapabilityBundleSelection] = useState<{
    pluginId: string;
    baseQuery: string;
  } | null>(null);
  ...
  const [composerCursor, setComposerCursor] = useState(() =>
    collapseExpandedComposerCursor(prompt, prompt.length, composerCapabilityMentions),
  );
  ...
  const addComposerCapabilityMentionToDraft = useCallback(
    (mention: ComposerCapabilityMention) => {
      addComposerDraftCapabilityMention(threadId, mention);
    },
    [addComposerDraftCapabilityMention, threadId],
  );
  const syncComposerCapabilityMentionsInDraft = useCallback(
    (mentions: ReadonlyArray<ComposerCapabilityMention>) => {
      syncComposerDraftCapabilityMentions(threadId, mentions);
    },
    [syncComposerDraftCapabilityMentions, threadId],
  );
  ...
  const capabilityTriggerQuery =
    composerTrigger?.kind === "capability" ? composerTrigger.query : "";
  const isPathTrigger = composerTriggerKind === "path";
  const isCapabilityTrigger = composerTriggerKind === "capability";
  const [debouncedPathQuery, composerPathQueryDebouncer] = useDebouncedValue(
    pathTriggerQuery,
    { wait: COMPOSER_PATH_QUERY_DEBOUNCE_MS },
    (debouncerState) => ({ isPending: debouncerState.isPending }),
  );
  const [debouncedCapabilityQuery, composerCapabilityQueryDebouncer] = useDebouncedValue(
    capabilityTriggerQuery,
    { wait: COMPOSER_CAPABILITY_QUERY_DEBOUNCE_MS },
    (debouncerState) => ({ isPending: debouncerState.isPending }),
  );
  const effectivePathQuery = pathTriggerQuery.length > 0 ? debouncedPathQuery : "";
  const effectiveCapabilityQuery =
    capabilityTriggerQuery.length > 0 ? debouncedCapabilityQuery : "";
  const capabilitySearchCwd = gitCwd ?? activeProject?.cwd ?? null;
  ...
  const capabilitySearchQuery = useQuery<CapabilitySearchResult>({
    ...
    queryFn: async () => {
      ...
      return api.capabilities.search({
        cwd: capabilitySearchCwd,
        query: effectiveCapabilityQuery,
        limit: COMPOSER_CAPABILITY_LIMIT,
      });
    },
    staleTime: 30_000,
  });
  const composerCapabilityBundleFilterQuery = useMemo(() => {
    if (!composerCapabilityBundleSelection) {
      return "";
    }
    const currentQuery = capabilityTriggerQuery.trim().toLowerCase();
    const baseQuery = composerCapabilityBundleSelection.baseQuery.trim().toLowerCase();
    if (baseQuery.length === 0) {
      return currentQuery;
    }
    if (currentQuery.startsWith(baseQuery)) {
      return currentQuery.slice(baseQuery.length).trim();
    }
    return currentQuery;
  }, [capabilityTriggerQuery, composerCapabilityBundleSelection]);
  const capabilityBundleQuery = useQuery<PluginBundleResult>({
    queryKey: [
      "composer-capability-bundle",
      capabilitySearchCwd ?? "__none__",
      composerCapabilityBundleSelection?.pluginId ?? "__none__",
    ],
    enabled: isCapabilityTrigger && composerCapabilityBundleSelection !== null,
    queryFn: async () => {
      const api = readNativeApi();
      if (!api || !composerCapabilityBundleSelection) {
        throw new Error("Capability bundle discovery is not available.");
      }
      return api.capabilities.readPluginBundle({
        pluginId: composerCapabilityBundleSelection.pluginId,
        cwd: capabilitySearchCwd,
      });
    },
    staleTime: 30_000,
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "capability") {
      if (composerCapabilityBundleSelection) {
        const bundle = capabilityBundleQuery.data;
        if (!bundle) {
          return [
            {
              id: `capability-back:${composerCapabilityBundleSelection.pluginId}`,
        ...
        const bundleQuery = composerCapabilityBundleFilterQuery;
        const skillItems = bundle.skills
          .filter((skill) => {
            if (!bundleQuery) return true;
            const searchable = [
              skill.name.toLowerCase(),
              skill.description.toLowerCase(),
              skill.pluginName.toLowerCase(),
            ];
            return searchable.some((value) => value.includes(bundleQuery));
          })
          .map(
            (skill) =>
              ({
                ...
                name: skill.name,
                path: skill.path,
                label: stripCapabilityPluginPrefix(skill.name, skill.pluginName),
                description: skill.description,
                scope: "plugin",
                enabled: skill.enabled,
                sourceLabel: `From ${bundle.plugin.name} plugin`,
                typeLabel: "Skill",
                groupLabel: "Plugin Skills",
                pluginId: skill.pluginId,
                pluginName: skill.pluginName,
              }) satisfies Extract<ComposerCommandItem, { type: "capability-skill" }>,
          );
        const appItems = bundle.apps
          .filter((app) => {
            if (!bundleQuery) return true;
            const searchable = [
              app.name.toLowerCase(),
              app.description.toLowerCase(),
              app.pluginName.toLowerCase(),
            ];
            return searchable.some((value) => value.includes(bundleQuery));
          })
          .map(
            (app) =>
              ({
                ...
                label: app.name,
                description: app.description.length > 0 ? app.description : "Connector app",
                sourceLabel: `From ${bundle.plugin.name} plugin`,
                typeLabel: "App",
                groupLabel: "Plugin Apps",
              }) satisfies Extract<ComposerCommandItem, { type: "capability-app" }>,
          );
        return [
          ...skillItems,
          ...appItems,
          {
            id: `capability-back:${bundle.plugin.id}`,
            type: "capability-back",
            label: "Back to all capabilities",
            description: `Return to ${bundle.plugin.name} and other capabilities`,
          },
        ];
      }

      const searchResult = capabilitySearchQuery.data;
      const skillItems =
        searchResult?.skills.map(
          (skill) =>
            ({
              ...
              scope: skill.scope,
              enabled: skill.enabled,
              sourceLabel: capabilitySkillSourceLabel(skill.scope, skill.pluginName),
              typeLabel: "Skill",
              groupLabel: "Skills",
      ...
      const pluginItems =
        searchResult?.plugins.map(
          (plugin) =>
            ({
              id: `capability-plugin:${plugin.id}`,
              ...
              description:
                plugin.description.length > 0
                  ? `${capabilityCountsLabel(plugin.bundleCounts.skills, plugin.bundleCounts.apps)} · ${plugin.description}`
                  : capabilityCountsLabel(plugin.bundleCounts.skills, plugin.bundleCounts.apps),
              sourceLabel: "Installed plugin",
              typeLabel: "Plugin",
              groupLabel: "Plugins",
              skillsCount: plugin.bundleCounts.skills,
              appsCount: plugin.bundleCounts.apps,
            }) satisfies Extract<ComposerCommandItem, { type: "capability-plugin" }>,
        ) ?? [];
    ...
    capabilityBundleQuery.data,
    capabilitySearchQuery.data,
    composerCapabilityBundleFilterQuery,
  ...
  const roomWorkspaceRoot = activeThreadWorktreePath ?? activeProjectCwd ?? undefined;
  ...
  const onViewModeChange = useCallback(
    (nextViewMode: "agent" | "room") => {
      ...
      void navigate({
        ...
        params: { threadId },
        replace: true,
        search: (previous) => ({ ...previous, view: nextViewMode }),
      });
    },
    [navigate, threadId, viewMode],
  );
  ...
  useEffect(() => {
    composerCapabilityMentionsRef.current = composerCapabilityMentions;
  }, [composerCapabilityMentions]);
  ...
  useEffect(() => {
    ...
    setComposerCursor((existing) =>
      clampCollapsedComposerCursor(prompt, existing, composerCapabilityMentions),
    );
  }, [composerCapabilityMentions, prompt]);

  useEffect(() => {
    if (composerTrigger?.kind !== "capability") {
      setComposerCapabilityBundleSelection(null);
    }
  }, [composerTrigger]);
  ...
  }, [composerCapabilityMentions, resetLocalDispatch, threadId]);
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    const composerCapabilityMentionsSnapshot = resolveCapabilityMentionsForPrompt(
      promptForSend,
      composerCapabilityMentionsRef.current,
    );
    ...
    await (async () => {
      ...
      await api.orchestration.dispatchCommand({
        ...
        message: {
          ...
          ...(composerCapabilityMentionsSnapshot.length > 0
            ? {
                capabilityMentions: composerCapabilityMentionsSnapshot,
              }
            : {}),
        },
      ...
      if (
        ...
        setComposerCursor(
          collapseExpandedComposerCursor(
            ...
            composerCapabilityMentionsSnapshot,
          ),
        );
        addComposerImagesToDraft(composerImagesSnapshot.map(cloneComposerImageForRetry));
        addComposerTerminalContextsToDraft(composerTerminalContextsSnapshot);
        syncComposerCapabilityMentionsInDraft(composerCapabilityMentionsSnapshot);
  ...
  const setPromptFromTraits = useCallback(
    ...
    [composerCapabilityMentions, scheduleComposerFocus, setPrompt],
  );
  ...
  const applyPromptReplacement = useCallback(
    (
      ...
      setComposerTrigger(
        detectComposerTrigger(
          next.text,
          expandCollapsedComposerCursor(next.text, nextCursor, composerCapabilityMentions),
        ),
      );
      window.requestAnimationFrame(() => {
  ...
  }, [composerCapabilityMentions, composerCursor, composerTerminalContexts]);
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "capability-back") {
        setComposerCapabilityBundleSelection(null);
        setComposerHighlightedItemId(null);
        return;
      }
      if (item.type === "capability-plugin") {
        setComposerCapabilityBundleSelection({
          pluginId: item.pluginId,
          baseQuery: trigger.kind === "capability" ? trigger.query : "",
        });
        setComposerHighlightedItemId(null);
        return;
      }
      if (item.type === "capability-skill") {
      ...
      if (item.type === "capability-app") {
        const tokenLabel = capabilityAppTokenLabel(item.name, item.appId);
        const replacement = `$${tokenLabel} `;
        const replacementRangeEnd = extendReplacementRangeForTrailingSpace(
          snapshot.value,
          trigger.rangeEnd,
          replacement,
  ...
  const isComposerMenuLoading =
    ...
    (composerTriggerKind === "capability" &&
      (composerCapabilityBundleSelection === null
        ? (capabilityTriggerQuery.length > 0 && composerCapabilityQueryDebouncer.state.isPending) ||
          capabilitySearchQuery.isLoading ||
          capabilitySearchQuery.isFetching
        : capabilityBundleQuery.isLoading || capabilityBundleQuery.isFetching));
  const composerMenuEmptyStateText =
    composerTriggerKind === "capability"
      ? composerCapabilityBundleSelection
        ? "No matching bundled skills or apps."
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      <section
        aria-hidden={!isAgentView}
        aria-label="Agent view"
        data-thread-surface="agent"
        hidden={!isAgentView}
                      ...
                      {composerMenuOpen && !isComposerApprovalState && (
                        <div className="absolute inset-x-0 bottom-full z-20 mb-2 px-1">
                          <ComposerCommandMenu
                            ...
                            {...(composerMenuEmptyStateText
                              ? { emptyStateText: composerMenuEmptyStateText }
                              : {})}
      ...
      {mountedTerminalThreadIds.map((mountedThreadId) => (
        <PersistentThreadTerminalDrawer
          ...
          visible={isAgentView && mountedThreadId === activeThreadId && terminalState.terminalOpen}
          launchContext={
