...
function buildBlockingCopy(
  ...
  if (uiState === "offline") {
    return {
      description: `Your browser is offline, so the web client cannot reach the ${APP_BASE_NAME} server. Reconnect to the network and the app will retry automatically.`,
      eyebrow: "Offline",
  ...
  if (status.lastError?.trim()) {
    return {
      description: `${status.lastError} Verify that the ${APP_BASE_NAME} server is running and reachable, then reload the app if needed.`,
      eyebrow: "Connection Error",
      title: `Cannot reach the ${APP_BASE_NAME} server`,
    };
  }

  return {
    description: `The web client could not complete its initial WebSocket connection to the ${APP_BASE_NAME} server. It will keep retrying in the background.`,
    eyebrow: "Connection Error",
    title: `Cannot reach the ${APP_BASE_NAME} server`,
  };
}
