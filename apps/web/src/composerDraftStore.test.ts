...
describe("composerDraftStore capability mentions", () => {
  const threadId = ThreadId.makeUnsafe("thread-capabilities");

  beforeEach(() => {
    resetComposerDraftStore();
  });

  it("keeps selected capability mentions while their token remains in the prompt", () => {
    useComposerDraftStore.getState().setPrompt(threadId, "Use $obsidian-cli please");
    useComposerDraftStore.getState().addCapabilityMention(threadId, {
      kind: "skill",
      token: "$obsidian-cli",
      label: "$obsidian-cli",
      name: "obsidian-cli",
    ...
    const draft = useComposerDraftStore.getState().draftsByThreadId[threadId];
    expect(draft?.capabilityMentions).toEqual([
      {
        kind: "skill",
        token: "$obsidian-cli",
        label: "$obsidian-cli",
  ...
  it("drops capability mentions when their token is removed from the prompt", () => {
    useComposerDraftStore.getState().setPrompt(threadId, "Use $obsidian-cli please");
    useComposerDraftStore.getState().addCapabilityMention(threadId, {
      kind: "skill",
      token: "$obsidian-cli",
    ...
    useComposerDraftStore.getState().setPrompt(threadId, "Use the local CLI please");
