...
export type ComposerTriggerKind = "path" | "slash-command" | "slash-model" | "capability";
...
export function expandCollapsedComposerCursor(
  ...
  const segments = splitPromptIntoComposerSegments(text, [], capabilityMentions);
  ...
  for (const segment of segments) {
    ...
    if (segment.type === "capability-mention") {
      const expandedLength = segment.mention.token.length;
      if (remaining <= 1) {
        return expandedCursor + (remaining === 0 ? 0 : expandedLength);
      }
      remaining -= 1;
...
export function clampCollapsedComposerCursor(
  ...
  return clampCollapsedComposerCursorForSegments(
    splitPromptIntoComposerSegments(text, [], capabilityMentions),
    cursorInput,
  );
}
