...
export const COMPOSER_DRAFT_STORAGE_KEY = "assist:composer-drafts:v1";
...
const PersistedComposerCapabilityMention = Schema.Union([
  Schema.Struct({
    token: Schema.String,
    label: Schema.String,
...
type PersistedComposerCapabilityMention = typeof PersistedComposerCapabilityMention.Type;

const PersistedComposerThreadDraftState = Schema.Struct({
  prompt: Schema.String,
  attachments: Schema.Array(PersistedComposerImageAttachment),
  terminalContexts: Schema.optionalKey(Schema.Array(PersistedTerminalContextDraft)),
  capabilityMentions: Schema.optionalKey(Schema.Array(PersistedComposerCapabilityMention)),
...
export interface ComposerThreadDraftState {
  ...
  capabilityMentions: ComposerCapabilityMention[];
...
interface ComposerDraftStoreState {
  ...
  addCapabilityMention: (threadId: ThreadId, mention: ComposerCapabilityMention) => void;
  syncCapabilityMentions: (
    threadId: ThreadId,
...
const EMPTY_CAPABILITY_MENTIONS: ComposerCapabilityMention[] = [];
Object.freeze(EMPTY_IMAGES);
Object.freeze(EMPTY_IDS);
Object.freeze(EMPTY_PERSISTED_ATTACHMENTS);
Object.freeze(EMPTY_CAPABILITY_MENTIONS);
...
const EMPTY_THREAD_DRAFT = Object.freeze<ComposerThreadDraftState>({
  ...
  capabilityMentions: EMPTY_CAPABILITY_MENTIONS,
...
function capabilityMentionDedupKey(mention: ComposerCapabilityMention): string {
  return mention.kind === "skill"
    ? `skill\u0000${mention.token}\u0000${mention.path}`
    : `app\u0000${mention.token}\u0000${mention.appId}`;
}

function normalizeCapabilityMention(value: unknown): ComposerCapabilityMention | null {
  if (!Schema.is(PersistedComposerCapabilityMention)(value)) {
    return null;
  }
  ...
  if (token.length === 0 || label.length === 0 || name.length === 0) {
    return null;
  }
  ...
  if (value.kind === "skill") {
    ...
    return {
      ...
      name,
      path: skillPath,
      ...(pluginId && pluginId.length > 0 ? { pluginId } : {}),
    };
  }
...
function normalizeCapabilityMentions(
  mentions: ReadonlyArray<ComposerCapabilityMention>,
): ComposerCapabilityMention[] {
  const seen = new Set<string>();
  const normalizedMentions: ComposerCapabilityMention[] = [];
  for (const mention of mentions) {
    const normalized = normalizeCapabilityMention(mention);
    if (!normalized) {
      continue;
    }
    const dedupKey = capabilityMentionDedupKey(normalized);
...
function syncCapabilityMentionsForPrompt(
  prompt: string,
  mentions: ReadonlyArray<ComposerCapabilityMention>,
): ComposerCapabilityMention[] {
  return normalizeCapabilityMentions(
    mentions.filter((mention) => mention.token.length > 0 && prompt.includes(mention.token)),
  );
}
...
function shouldRemoveDraft(draft: ComposerThreadDraftState): boolean {
  return (
    ...
    draft.capabilityMentions.length === 0 &&
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadId, draftValue] of Object.entries(rawDraftMap as Record<string, unknown>)) {
    ...
    const terminalContexts = Array.isArray(draftCandidate.terminalContexts)
      ...
      : [];
    const rawCapabilityMentions = Array.isArray(draftCandidate.capabilityMentions)
      ? normalizeCapabilityMentions(
          draftCandidate.capabilityMentions.flatMap((entry) => {
            const normalized = normalizeCapabilityMention(entry);
            return normalized ? [normalized] : [];
          }),
        )
      : [];
    ...
    const capabilityMentions = syncCapabilityMentionsForPrompt(prompt, rawCapabilityMentions);
    ...
    nextDraftsByThreadId[threadId as ThreadId] = {
      ...
      ...(capabilityMentions.length > 0 ? { capabilityMentions } : {}),
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadId, draft] of Object.entries(state.draftsByThreadId)) {
    ...
    const persistedDraft: DeepMutable<PersistedComposerThreadDraftState> = {
      ...
      ...(draft.capabilityMentions.length > 0
        ? {
            capabilityMentions: draft.capabilityMentions,
          }
        : {}),
      ...(hasModelData
        ? {
...
function toHydratedThreadDraft(
  ...
  return {
    ...
    capabilityMentions: [...(persistedDraft.capabilityMentions ?? [])],
    modelSelectionByProvider,
    activeProvider,
...
export const useComposerDraftStore = create<ComposerDraftStoreState>()(
  persist(
    (set, get) => ({
      ...
      setPrompt: (threadId, prompt) => {
        ...
        set((state) => {
          ...
          const nextDraft: ComposerThreadDraftState = {
            ...existing,
            prompt,
            capabilityMentions: syncCapabilityMentionsForPrompt(
              prompt,
              existing.capabilityMentions,
            ),
          };
          const nextDraftsByThreadId = { ...state.draftsByThreadId };
          if (shouldRemoveDraft(nextDraft)) {
      ...
      addCapabilityMention: (threadId, mention) => {
        if (threadId.length === 0) {
          return;
        }
        set((state) => {
          ...
          const nextDraft: ComposerThreadDraftState = {
            ...existing,
            capabilityMentions: syncCapabilityMentionsForPrompt(existing.prompt, [
              ...existing.capabilityMentions,
              mention,
            ]),
          };
      ...
      syncCapabilityMentions: (threadId, mentions) => {
        if (threadId.length === 0) {
          return;
        }
        set((state) => {
          const existing = state.draftsByThreadId[threadId];
          const normalizedMentions = existing
            ? syncCapabilityMentionsForPrompt(existing.prompt, mentions)
            : [];
          if (!existing && normalizedMentions.length === 0) {
            return state;
          }
          const nextDraft: ComposerThreadDraftState = {
            ...(existing ?? createEmptyThreadDraft()),
            capabilityMentions: normalizedMentions,
          };
          const nextDraftsByThreadId = { ...state.draftsByThreadId };
          if (shouldRemoveDraft(nextDraft)) {
            delete nextDraftsByThreadId[threadId];
