...
describe("detectComposerTrigger", () => {
  ...
  it("detects $capability trigger at cursor", () => {
    const text = "Use $obsidian";
    const trigger = detectComposerTrigger(text, text.length);
  ...
  it("detects bare $capability trigger in the middle of text", () => {
    const text = "Use $ inside this sentence";
    const cursorAfterDollar = "Use $".length;

    const trigger = detectComposerTrigger(text, cursorAfterDollar);
    expect(trigger).toEqual({
      kind: "capability",
      query: "",
      rangeStart: "Use ".length,
      rangeEnd: cursorAfterDollar,
    });
  });
});
...
describe("expandCollapsedComposerCursor", () => {
  ...
  it("treats selected skill mentions as single inline tokens", () => {
    const text = "Use $obsidian-cli please";
    const collapsedCursorAfterMention = "Use ".length + 2;

    expect(
      expandCollapsedComposerCursor(text, collapsedCursorAfterMention, [OBSIDIAN_SKILL_MENTION]),
    ).toBe("Use $obsidian-cli ".length);
  });
});
...
describe("collapseExpandedComposerCursor", () => {
  ...
  it("collapses selected skill mentions back to a single cursor step", () => {
    const text = "Use $obsidian-cli please";

    expect(
      collapseExpandedComposerCursor(text, "Use $obsidian-cli ".length, [OBSIDIAN_SKILL_MENTION]),
    ).toBe("Use ".length + 2);
  });
});
...
describe("isCollapsedCursorAdjacentToInlineToken", () => {
  ...
  it("treats selected skill chips as inline tokens for adjacency checks", () => {
    const text = "Use $obsidian-cli next";
    const tokenStart = "Use ".length;
    const tokenEnd = tokenStart + 1;

    expect(
      isCollapsedCursorAdjacentToInlineToken(text, tokenEnd, "left", [OBSIDIAN_SKILL_MENTION]),
    ).toBe(true);
    expect(
      isCollapsedCursorAdjacentToInlineToken(text, tokenStart, "right", [OBSIDIAN_SKILL_MENTION]),
    ).toBe(true);
  });
});
