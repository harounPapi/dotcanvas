...
export interface WsRpcClient {
  ...
  readonly capabilities: {
    ...
    readonly readPluginBundle: RpcUnaryMethod<typeof WS_METHODS.capabilitiesReadPluginBundle>;
  };
  readonly shell: {
...
export function createWsRpcClient(transport = new WsTransport()): WsRpcClient {
  return {
    ...
    capabilities: {
      search: (input) =>
        transport.request((client) => client[WS_METHODS.capabilitiesSearch](input)),
      readPluginBundle: (input) =>
        transport.request((client) => client[WS_METHODS.capabilitiesReadPluginBundle](input)),
    },
    shell: {
      openInEditor: (input) =>
