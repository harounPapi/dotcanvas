...
const makeTempDir = Effect.fn(function* (opts?: { prefix?: string; git?: boolean }) {
  ...
  const dir = yield* fileSystem.makeTempDirectoryScoped({
    prefix: opts?.prefix ?? "assist-workspace-entries-",
  });
  if (opts?.git) {
...
it.layer(TestLayer)("WorkspaceEntriesLive", (it) => {
  ...
  describe("listDirectory", () => {
    it.effect("lists direct child entries and keeps directories ahead of files", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-list-root-" });
    ...
    it.effect("lists nested directory contents relative to the workspace root", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-list-nested-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("fails for missing and non-directory paths", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-list-errors-" });
        yield* writeTextFile(cwd, "src/index.ts");
    ...
    it.effect("filters ignored and gitignored direct child entries", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-list-ignore-", git: true });
  ...
  describe("search", () => {
    ...
    it.effect("filters and ranks entries by query", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-query-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("supports fuzzy subsequence queries for composer path search", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-fuzzy-query-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
        yield* writeTextFile(cwd, "src/components/composePrompt.ts");
        yield* writeTextFile(cwd, "docs/composition.md");
    ...
    it.effect("tracks truncation without sorting every fuzzy match", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-fuzzy-limit-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
        yield* writeTextFile(cwd, "src/components/composePrompt.ts");
        yield* writeTextFile(cwd, "docs/composition.md");
    ...
    it.effect("excludes gitignored paths for git repositories", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-gitignore-", git: true });
    ...
    it.effect("excludes tracked paths that match ignore rules", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({
          prefix: "assist-workspace-tracked-gitignore-",
          git: true,
        });
        yield* writeTextFile(cwd, ".convex/local-storage/data.json", "{}");
        yield* writeTextFile(cwd, "src/keep.ts", "export {};");
    ...
    it.effect("excludes .convex in non-git workspaces", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-non-git-convex-" });
        yield* writeTextFile(cwd, ".convex/local-storage/data.json", "{}");
        yield* writeTextFile(cwd, "src/keep.ts", "export {};");
    ...
    it.effect("deduplicates concurrent index builds for the same cwd", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-concurrent-build-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("limits concurrent directory reads while walking the filesystem", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "assist-workspace-read-concurrency-" });
