...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  ...
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "assist-codex-text-generation-test-",
    }),
  ),
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__ASSIST_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__ASSIST_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "assist-codex-text-" });
