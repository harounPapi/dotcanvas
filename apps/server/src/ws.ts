...
import { CapabilityCatalog } from "./capabilities/Services/CapabilityCatalog.ts";
...
const WsRpcLayer = WsRpcGroup.toLayer(
  Effect.gen(function* () {
    ...
    const capabilityCatalog = yield* CapabilityCatalog;
    ...
    return WsRpcGroup.of({
      ...
      [WS_METHODS.capabilitiesSearch]: (input) =>
        observeRpcEffect(WS_METHODS.capabilitiesSearch, capabilityCatalog.search(input), {
          "rpc.aggregate": "capabilities",
        }),
      [WS_METHODS.capabilitiesReadPluginBundle]: (input) =>
        observeRpcEffect(
          WS_METHODS.capabilitiesReadPluginBundle,
          capabilityCatalog.readPluginBundle(input),
          {
            "rpc.aggregate": "capabilities",
          },
        ),
