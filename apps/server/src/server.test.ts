...
import {
  CapabilityCatalog,
  type CapabilityCatalogShape,
} from "./capabilities/Services/CapabilityCatalog.ts";
import {
  CheckpointDiffQuery,
...
import { WorkspaceDocumentFileSystemLive } from "./workspace/Layers/WorkspaceDocumentFileSystem.ts";
...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const appLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        Layer.mock(CapabilityCatalog)({
          search: () => Effect.succeed({ skills: [], plugins: [] }),
          readPluginBundle: (input) =>
            Effect.succeed({
              plugin: {
                id: input.pluginId,
                name: input.pluginId,
                description: "",
                marketplacePath: input.pluginId,
                pluginName: input.pluginId,
                installed: true,
                bundleCounts: { skills: 0, apps: 0 },
              },
              skills: [],
              apps: [],
            }),
          ...options?.layers?.capabilityCatalog,
        }),
      ),
      Layer.provide(
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("routes websocket rpc projects.listDirectory errors", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "t3-ws-project-list-directory-errors-",
      });
      yield* fs.makeDirectory(path.join(workspaceDir, "src"), { recursive: true });
      yield* fs.writeFileString(path.join(workspaceDir, "src", "index.ts"), "export {};");
