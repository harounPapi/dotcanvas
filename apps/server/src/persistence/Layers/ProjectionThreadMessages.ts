...
import { ChatAttachment, ComposerCapabilityMention } from "@t3tools/contracts";
...
const ProjectionThreadMessageDbRowSchema = ProjectionThreadMessage.mapFields(
  Struct.assign({
    ...
    capabilityMentions: Schema.NullOr(
      Schema.fromJsonString(Schema.Array(ComposerCapabilityMention)),
    ),
  }),
);
...
function toProjectionThreadMessage(
  ...
  return {
    ...
    ...(row.capabilityMentions !== null ? { capabilityMentions: row.capabilityMentions } : {}),
  };
}
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextCapabilityMentionsJson =
        row.capabilityMentions !== undefined ? JSON.stringify(row.capabilityMentions) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextCapabilityMentionsJson},
            (
              SELECT capability_mentions_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          capability_mentions_json = COALESCE(
            excluded.capability_mentions_json,
            projection_thread_messages.capability_mentions_json
          ),
    ...
    execute: ({ messageId }) =>
      sql`
        SELECT
          ...
          capability_mentions_json AS "capabilityMentions",
