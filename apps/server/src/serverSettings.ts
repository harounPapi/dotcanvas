...
function resolveTextGenerationProvider(settings: ServerSettings): ServerSettings {
  ...
  const requestedProviderSupported = PROVIDER_ORDER.includes(selection.provider);
  if (requestedProviderSupported && settings.providers[selection.provider].enabled) {
    return settings;
  }
