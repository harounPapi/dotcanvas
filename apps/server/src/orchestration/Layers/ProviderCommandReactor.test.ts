...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "assist-reactor-"));
  ...
  it("forwards capability mentions through turn send", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-capabilities"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-capabilities"),
          role: "user",
          text: "Use $obsidian-cli and $github",
          attachments: [],
          capabilityMentions: [
            {
              kind: "skill",
    ...
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
      threadId: ThreadId.makeUnsafe("thread-1"),
      input: "Use $obsidian-cli and $github",
      capabilityMentions: [
        {
          kind: "skill",
          token: "$obsidian-cli",
